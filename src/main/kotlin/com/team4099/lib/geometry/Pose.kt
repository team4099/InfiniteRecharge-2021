...
data class Pose(val x: Length, val y: Length, val theta: Angle) {
  constructor(translation: Translation, theta: Angle) :
    this(translation.x, translation.y, theta)
  ...
  fun relativeTo(other: Pose) : Pose{
    return Pose(
      pose2d.relativeTo(other.pose2d)
    )
  }
  ...
  fun transformBy(x: Length, y: Length, theta: Angle) : Pose{
    return Pose(
      pose2d.transformBy(
        Transform2d(
          Translation2d(x.inMeters, y.inMeters),
          Rotation2d(theta.inRadians)
        )
      )
    )
  }

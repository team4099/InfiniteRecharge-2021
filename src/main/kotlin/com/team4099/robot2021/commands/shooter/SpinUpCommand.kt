...
class SpinUpCommand(private val withVision: Boolean = false, private val accuracy: Boolean = false, private val distance: Vision.DistanceState = Vision.DistanceState.NEAR) : CommandBase() {
  init {
  ...
  override fun execute() {
    if (withVision) {
      ...
      if (accuracy) {
        Shooter.targetVelocity = when (distance) {
            Vision.DistanceState.LINE -> Constants.Shooter.LINE_VELOCITY
            Vision.DistanceState.NEAR -> Constants.Shooter.NEAR_VELOCITY
            Vision.DistanceState.MID -> Constants.Shooter.MID_VELOCITY
            Vision.DistanceState.FAR -> Constants.Shooter.FAR_VELOCITY
          }
        Shooter.hoodState = when (distance) {
